...
abstract interface class _LosslessTransform {
  ...
  int encodedWidth(int width) => width;
  ...
  static _LosslessTransform read(_BitReader reader, int width, int height) {
    ...
    return switch (type) {
      ...
      3 => _ColorIndexingTransform.read(reader, width),
...
final class _LosslessImage {
  const _LosslessImage(this.bytes, this.width, this.height);
